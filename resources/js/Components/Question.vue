...
const canUseAddButton = computed(() => {
    ...
    if (simpleInputs.includes(props.questionObject.type)) {
        return props.questionObject.title?.length > 0;
    } else {
        return (
            props.questionObject?.title?.length > 0 &&
            props.questionObject?.options?.length > 0
        );
    }
});
